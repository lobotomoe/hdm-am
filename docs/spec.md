...
> - **AS** (ԱԾ, Առևտրային Ծրագիր, "Commercial Software/Application") — the external application that integrates with the HDM. In this crate's context, that is the consumer of `hdm-am`.
...
| Acronym | Description |
|---|---|
| AS | Commercial Software/Application (Armenian: Առևտրային Ծրագիր) |
...
- *Տպել կտրոնը ՀԴՄ տպիչով* — "Print receipt on HDM printer" (checkbox)
...
3. **Automatic system IP address** — the IP address of the AS's computer; the HDM will exchange data with it.
...
6. **Port** — the HDM port number that the AS will use to connect to the HDM. The port number is entered on the HDM keyboard.
...
|---|---|---|
| **P** | Working with printer | The HDM generates and stores the fiscal receipt and prints it on its own printer. |
| **R** | Working without printer | The HDM generates and stores the fiscal receipt and returns the receipt's fiscal data. |
...
6. Send another message or terminate the connection.
7. The maximum request size must not exceed 50 kilobytes.
...
|---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|---|
| Hex | `00` | `05` | `02` | `02` | `10` | `02` | `00` | `00` | `C8` | `00` | `00` | … |
...
> **Spec caveat.** The PDF's literal hex sample and field labels are internally inconsistent with a normal response-code/payload-length interpretation. The translated table preserves the PDF bytes and labels; this crate's `wire::ResponseHeader::read` keeps the hardware-observed 11-byte parser: protocol version (2 bytes), software version (3 bytes), response code (2 bytes BE), payload length (2 bytes BE), then 2 reserved bytes (skipped).
...
During an established session, each request includes a request sequence number. It is mandatory and must conform to the following requirements:

1. The first request can send any integer as the sequence number.
2. For each subsequent request, the sequence number must be greater than the sequence number sent with the previous request.
...
| `time` | Long | Receipt registration/print date and time |
...
This operation prints a copy of the last receipt, including the header and footer.
...
> **Translator's note.** The operation lists name code 10 generically as *"Կտրոնի տվյալների ստացում"* / *"ՀԴՄ կտրոնի տեղեկությունների ստացում"* — **get receipt data / information**. The detailed section title is *"ՀԴՄ վերադարձվող կտրոնի ստացում"* — **get the returnable receipt**, not "print return receipt". The wire operation code is **10** per the operation-codes table (§4.4.1) — the section number is not the operation code. This is a **read-only lookup**: given a receipt number it returns that receipt's full fiscal contents (items, amounts, eMarks, sale type) so the integrator can build the actual return via op 6 (§4.5.7). It registers nothing. (The receipt looked up is the *վերադարձվող* one — "the one to be returned", i.e. the original sale; op 6 by contrast prints the *վերադարձի* receipt — the new refund document.)
...
> **Spec caveat.** The request field table spells the lookup key as `receiptID`, while the PDF example spells it as `receiptId`. The crate uses `receiptId`, matching the example and the naming style used elsewhere in the manual.
...
| Field | Type | Description |
|---|---|---|
| `rseq` | Long | Return receipt sequence number (PDF wording; Code Block 7 omits this field) |
...
| `ref` | String | Number of the receipt being returned |
...
| `>qty` | String | Quantity |
| `>p` | String | Unit price |
| `>adg` | String | ATG/FEACN product code |
...
```json
{
  ...
  "totals": [
    ...
    {
      "gc": "001", "gn": "Product1", "qty": 1, "p": 1000, "mu": "տուփ",
      "rpid": 1, "dsc": null, "adsc": null, "dsct": null,
      "did": 1, "dt": 16.67, "dtm": 1, "t": 833.33, "tt": 1000.00
    }
  ]
}
...
| `cashAmountForReturn` | BigDecimal | Cash amount to be returned (set only if a partial return is performed) |
| `cardAmountForReturn` | BigDecimal | Cashless (card) amount to be returned (set only if a partial return is performed) |
| `prePaymentAmountForReturn` | BigDecimal | Prepayment used amount to be returned (set only if a partial return is performed) |
| `rrn` | String | Transaction unique identifier (12 chars) |
| `terminalId` | String | Payment terminal unique number (8 chars) |
| `eMarks` | Array | The eMark codes of marked goods in the receipt, in string-array form. Sent only for sale receipts. The length of each eMark code sent to the HDM must be at least **29** characters (inclusive) and at most **110** characters (inclusive). eMark codes must contain ASCII-table printable symbols only — from ASCII code 33 (inclusive) up to ASCII code 126 (inclusive) and ASCII code 29: inside the code, any `"` character must be escaped as `\"`, any `\` character must be escaped as `\\`, and ASCII code 29 must be escaped as `\u001d`. |
| `returnItemList` | Array | List of return items. **Sent only when partially returning an itemised receipt.** Each row is described in the following `returnItemList` description |
...
| `taxpayer` | String | Taxpayer's name |
| `address` | String | Taxpayer's address |
| `time` | Long | Registration/print date and time of the receipt being returned, Greenwich time |
...
| `deptId` | Integer | Department selection: if the report is generated without this selection type, the field should be omitted. Only one filter can be used. |
| `cashierId` | Integer | Cashier selection: if the report is generated without this selection type, the field should be omitted. Only one filter can be used. |
| `transactionTypeId` | Integer | Payment method — cash or cashless: if the report is generated without this selection type, the field should be omitted. Only one filter can be used. |
...
|---|---|---|
| `seq` | Integer | Request sequence number |
| `eMark` | String | eMark code. The length of the eMark code sent to the HDM must be at least **29** characters (inclusive) and at most **110** characters (inclusive). The eMark code must contain ASCII-table printable symbols only — from ASCII code 33 (inclusive) up to ASCII code 126 (inclusive) and ASCII code 29: inside the code, any `"` character must be escaped as `\"`, any `\` character must be escaped as `\\`, and ASCII code 29 must be escaped as `\u001d`. |
...
| 400 | Request error | Returned when the request cannot be decoded | X |
| 402 | Bad protocol version | | X |
| 403 | Unauthorised connection | Returned when the IP address configured in the HDM does not match the IP address of the connecting device | X |
| 404 | Bad operation code | Returned when the operation code specified in the header is wrong | X |
| 101 | Password-key encryption error | | X |
...
| 180 | Full-return amount is too high | | |
